...
resin_conversion_func_new <- function(bg_material_type_df, resin_conversion_df) {
  ...
  process_column <- function(column_name) {
    df <- bg_material_type_df %>%
      select(county, business_group_name, employee_count, resin_conversion, all_of(column_name)) %>%
      group_by(county, business_group_name, employee_count, resin_conversion) %>%
      ...
      ungroup() %>%
      mutate(across(!c(county, business_group_name, employee_count), as.numeric)) %>% 
    ...
    df <- df %>%
      ...
      select(county, business_group_name, employee_count, PET, HDPE, PP, LDPE_LLDPE, PVC, Other_Resins, PS) %>%
...
converted_resin_df <- resin_conversion_func_new(bg_material_type_df_clean, resin_conversion_df)
```

```{r}
write_xlsx(converted_resin_df, here("processed data/WCS Business Group by Resin.xlsx"))
```
